...
// ==== KIT MODEL START ====
// ── Kit model ───────────────────────────────────────────────────────────────
// Everything the plugin knows about the Obra shadcn/ui kit's structure, as pure
// functions over names and strings. No Figma API calls live here, so this whole
// section is testable under plain node — which is how the generic scanner's
// matcher bugs were caught.
//
// Derived from ~/Sites/obra-shadcn-ui-kit/figma/CLAUDE.md. That file is the
// source of truth; when it changes, change this.
//
// This file is concatenated into code.js at the top. It is kept separate on disk
// only so the tests can load it on its own.

// The 8 Pro styles. Nova is the lead file; Community tracks the Nova style.
var KIT_STYLES = ['Nova', 'Luma', 'Vega', 'Rhea', 'Sera', 'Lyra', 'Mira', 'Maia'];

// Libraries that look like the kit but are not one of the 9. A hit from these is
// contamination, not drift, and is never a rebind target.
var KIT_FOREIGN_LIBRARY_HINTS = [
  { match: /\(Daphne\)/i, kind: 'daphne',
    note: 'a 10th kit library carrying the same version and components — out of scope' },
  { match: /AI Digital Elevate UI Library/i, kind: 'client-fork', note: 'client fork' },
  { match: /LaluCARE/i, kind: 'client-fork', note: 'client fork' }
];

// Components that are deliberately Pro-only. Anchored at the START of the name:
// "Todo Marker (OC)" is not the chat "Marker".
var KIT_PRO_ONLY_COMPONENTS = ['Attachment', 'Bubble', 'Marker', 'Message', 'Message Scroller'];

// Pages documenting tokens rather than components. They must not carry a footer,
// and they are not expected to hold component doc frames.
var KIT_TOKEN_PAGES = ['Colors', 'Typography', 'Icons', 'Shadows'];

// Helpers that live on the Internal Components page and are referenced from every
// component page. They are `.`-prefixed, so they never publish.
var KIT_INTERNAL_HELPERS = [
...
// ── File identity ───────────────────────────────────────────────────────────
// "Obra shadcn/ui kit - Pro edition (1.14.0) - Nova"
// "Obra shadcn/ui kit - Community edition (2.0.0) - Nova (WIP. Dont republish…)"
function kitIdentifyFile(fileName) {
  var name = String(fileName || '');
  var edition = /Community edition/i.test(name) ? 'Community'
    : /Pro edition/i.test(name) ? 'Pro'
    : null;

  var version = null;
  var versionMatch = name.match(/\((\d+\.\d+\.\d+)\)/);
  if (versionMatch) version = versionMatch[1];

  // The style is the last kit style word that appears in the name. Community
  // files read "… - Nova (WIP…)", so a trailing parenthetical must not hide it.
  var style = null;
  for (var i = 0; i < KIT_STYLES.length; i++) {
    var re = new RegExp('(^|[^A-Za-z])' + KIT_STYLES[i] + '([^A-Za-z]|$)');
    if (re.test(name)) style = KIT_STYLES[i];
  }
  ...
  return {
    ...
    edition: edition,
    version: version,
    isKitFile: !!(style && edition)
  };
}

// ── Where a remote reference came from ──────────────────────────────────────
// `libraryName` is only available for variable collections (via teamLibrary);
// pass null for components and styles and the naming rules carry the weight.
function kitClassifySource(refName, libraryName, thisStyle) {
  var lib = String(libraryName || '');

  for (var i = 0; i < KIT_FOREIGN_LIBRARY_HINTS.length; i++) {
    var hint = KIT_FOREIGN_LIBRARY_HINTS[i];
    if (hint.match.test(lib)) {
      return { kind: hint.kind, style: null, note: hint.note };
    }
  }

  // A library name states the style outright.
  if (lib) {
    for (var s = 0; s < KIT_STYLES.length; s++) {
      var styleRe = new RegExp('(^|[^A-Za-z])' + KIT_STYLES[s] + '([^A-Za-z]|$)');
      if (styleRe.test(lib)) {
        var style = KIT_STYLES[s];
        return style === thisStyle
          ? { kind: 'self', style: style,
              note: 'this file’s own published library — still forbidden, the file must be self-contained' }
          : { kind: 'sibling-style', style: style,
              note: 'another style’s library — this is drift, not a style decision' };
      }
    }
    return { kind: 'unknown', style: null, note: 'library "' + lib + '" is not part of the kit' };
  }

  // No library name: read the style suffix off the component name instead.
  var suffix = kitStyleSuffixOf(refName);
  if (suffix) {
    return suffix === thisStyle
      ? { kind: 'self', style: suffix,
          note: 'named for this style, so it is this file’s own library' }
      : { kind: 'sibling-style', style: suffix,
          note: 'named for the ' + suffix + ' style — this is drift' };
  }

  if (kitLucideScheme(refName) === 'unpublished') {
    return { kind: 'lucide-full-set', style: null,
             note: 'from the full Lucide set, which is not present in this file' };
  }
  if (kitLucideScheme(refName) === 'published') {
    return { kind: 'lucide-published', style: null, note: 'from the published icon set' };
  }

  return { kind: 'unknown', style: null, note: null };
}

function kitStyleSuffixOf(name) {
  var m = String(name || '').match(/\s-\s([A-Za-z]+)\s*$/);
  if (!m) return null;
  for (var i = 0; i < KIT_STYLES.length; i++) {
    if (KIT_STYLES[i].toLowerCase() === m[1].toLowerCase()) return KIT_STYLES[i];
  }
  return null;
}

// `Lucide / x`  → 'published'   (has a local `Icon / x`)
// `Lucide  / x` → 'unpublished' (double space; the full set, absent locally)
function kitLucideScheme(name) {
  var value = String(name || '');
  if (/^Lucide {2,}\/\s*/.test(value)) return 'unpublished';
  if (/^Lucide \/\s*/.test(value)) return 'published';
  return null;
}

// ── Deterministic rewrites ──────────────────────────────────────────────────
// Candidate local names for a remote reference, most trustworthy first. These
// encode published structural rules, so a hit is `exact` — not a guess that
// happens to score well.
function kitRewriteCandidates(refName, thisStyle) {
  var name = String(refName || '');
  var out = [];
  var add = function (candidate, why) {
    if (!candidate || candidate === name) return;
    for (var i = 0; i < out.length; i++) if (out[i].name === candidate) return;
    out.push({ name: candidate, why: why });
  };

  // Rule 1 — style suffix. "Button - Nova" inside Luma is "Button - Luma".
  var suffix = kitStyleSuffixOf(name);
  if (suffix && thisStyle && suffix !== thisStyle) {
    add(name.replace(/\s-\s[A-Za-z]+\s*$/, ' - ' + thisStyle),
        'style-ified components are named "<Component> - <Style>", so the ' + suffix +
        ' reference maps to the ' + thisStyle + ' one');
  }

  // Rule 2 — published Lucide icons are re-published locally as `Icon / x`.
  if (kitLucideScheme(name) === 'published') {
    add(name.replace(/^Lucide \/\s*/, 'Icon / '),
        'the kit republishes `Lucide / x` as `Icon / x`');
  }

  return out;
}

// Why a reference cannot be rebound, when the reason is structural rather than a
// failure to find a name. Returning something here means "stop looking for a
// match" — no matcher can succeed.
function kitUnfixableReason(refName) {
  if (kitLucideScheme(refName) === 'unpublished') {
    return {
      code: 'lucide-set-missing',
      message: 'the full Lucide set is not in this file — only ~111 icons are published locally as ' +
        '`Icon / …`. This is why Figma’s Swap Library fails on these: there is no local target. ' +
        'Import the icon set, or leave these until it lands.'
    };
  }
  return null;
}
...
function kitIsDividerPage(pageName) {
  var name = String(pageName || '').trim();
  return name === '' || /^-+$/.test(name) || /^📖/.test(name);
}

function kitIsTokenPage(pageName) {
  return KIT_TOKEN_PAGES.indexOf(String(pageName || '').trim()) !== -1;
}

// The component a page documents, with its markers stripped. "Button & Icon
// Button •" documents "Button & Icon Button"; "Backdrop (OC)" is Obra Custom.
function kitComponentFromPageName(pageName) {
  var raw = String(pageName || '').trim();
  if (!raw || kitIsDividerPage(raw) || kitIsTokenPage(raw)) return null;
  if (/^\(Template\)$/i.test(raw)) return null;
  if (/^Internal Components$/i.test(raw)) return null;
  if (/^Pro blocks/i.test(raw)) return null;

  var styleified = /\s•\s*$/.test(raw);
  var base = raw.replace(/\s•\s*$/, '').trim();
  var isObraCustom = /\(OC\)\s*$/.test(base);

  return {
    page: raw,
    name: base,
    baseName: base.replace(/\s*\(OC\)\s*$/, '').trim(),
    styleified: styleified,
    obraCustom: isObraCustom,
    // Sort key per the kit rule: ignore the trailing marker and any leading emoji.
    sortKey: base.replace(/^[^\p{L}\p{N}]+/u, '').toLowerCase()
  };
}

// Pro-only by deliberate decision, so its absence from Community is not a gap.
function kitIsProOnly(componentName) {
  var name = String(componentName || '').trim();
  for (var i = 0; i < KIT_PRO_ONLY_COMPONENTS.length; i++) {
    var only = KIT_PRO_ONLY_COMPONENTS[i];
    // Anchored at the start: "Todo Marker (OC)" is not the chat "Marker".
    if (name === only || name.indexOf(only + ' ') === 0 || name.indexOf(only + ' -') === 0) return true;
  }
  return false;
}

function kitIsInternalHelper(name) {
  return KIT_INTERNAL_HELPERS.indexOf(String(name || '').trim()) !== -1 ||
    String(name || '').charAt(0) === '.';
}

// A page that documents a component should hold a doc frame named for it, and a
// footer — except on the token pages.
function kitExpectedDocFrame(componentName, style) {
  if (!componentName) return null;
  return style ? componentName + ' - ' + style : componentName;
}

function kitExpectsFooter(pageName) {
  if (kitIsTokenPage(pageName)) return false;
  return !!kitComponentFromPageName(pageName);
}
// ==== KIT MODEL END ====
...
figma.showUI(__html__, {
  ...
  height: 660,
  themeColors: true,
  title: 'Obra Remote Library Scanner — shadcn kit'
});
...
const DEFAULT_SETTINGS = {
  ...
  // 'overrides' — also inspect descendants the file genuinely overrode
  // 'deep'      — walk every instance descendant (slow; matches the old engine) (default)
  //
  // 'overrides' silently misses remote references that are unmodified, inherited
  // content — the exact shape produced when a whole component tree is pasted in
  // from another file's library and never re-localised. Verified on the Community
  // edition file: 'overrides' mode reported 0 remote refs on Menubar/Sidebar/
  // Navigation Menu/Item/Empty/Data Table, while a 'deep' walk found 217 across 25
  // groups — all genuine, all just inherited (non-overridden) pasted content.
  depth: 'deep',
...
// Which of the 9 kit files this is. The style drives the rename rules, so it is
// resolved once and cached.
let kitFile = null;
function kitThisFile() {
  if (!kitFile) kitFile = kitIdentifyFile(figma.root.name);
  return kitFile;
}
...
function computeMatch(kind, remoteName, preferPageIds) {
  ...
  // ── Kit rules first ──────────────────────────────────────────────────────
  // These encode published structure, so a hit is exact rather than inferred.
  // Without them the generic tiers cannot see these pairs at all: "Button - Nova"
  // vs "Button - Luma" scores 0.69, below any safe fuzzy threshold.
  const kit = kitThisFile();
  if (kit.isKitFile) {
    const rewrites = kitRewriteCandidates(remoteName, kit.style);
    for (const rewrite of rewrites) {
      const direct = set.byName.get(rewrite.name) ||
        set.byNormalised.get(normaliseName(rewrite.name));
      if (direct && direct.length) {
        const target = resolveMatchTarget(direct[0]);
        if (target) {
          const page = kind === 'component' ? pageOfComponent(direct[0]) : null;
          return {
            targetId: target.id,
            targetName: direct[0].name,
...
// ── Kit structure audit ─────────────────────────────────────────────────────
// The generic scanner reports what it FINDS. This reports what is ABSENT — a
// component page with no component on it, a doc frame under the wrong name, a
// footer where the kit forbids one. None of that produces a remote reference, so
// no amount of scanning surfaces it.

async function handleKitAudit() {
  const gen = beginRun();
  try {
    const kit = kitThisFile();
    if (!kit.isKitFile) {
      figma.ui.postMessage({
        type: 'kit-report', report: null,
        error: 'This file is not one of the 9 kit files — its name carries no style and edition. ' +
          'The structural rules do not apply here.'
      });
      return;
    }

    postProgress({ message: 'Reading page structure…', processed: 0, total: 0, cancellable: true });
    await ensureAllPages();
    if (isStale(gen)) return;
    ...
    for (let i = 0; i < pages.length; i++) {
      if (isStale(gen)) return;
      const page = pages[i];
      const expected = kitComponentFromPageName(page.name);
      ...
      // The doc frame is named "<Component> - <Style>" for style-ified
      // components, or plainly for the rest.
      const wantStyled = kitExpectedDocFrame(expected.name, kit.style);
      const wantPlain = expected.name;
      const docFrame = children.find(child =>
        child.name === wantStyled || child.name === wantPlain) || null;

      // Does anything on the page actually define this component?
      let hasComponent = false;
      const walk = (node, depth) => {
        if (hasComponent || depth > 4) return;
        if (node.type === 'COMPONENT' || node.type === 'COMPONENT_SET') { hasComponent = true; return; }
      ...
      const record = {
        ...
        pageId: page.id,
        name: expected.name,
        styleified: expected.styleified,
        obraCustom: expected.obraCustom,
        hasComponent,
        hasDocFrame: !!docFrame,
        docFrameName: docFrame ? docFrame.name : null
      };
      ...
      if (!hasComponent) {
        findings.push({
          severity: 'missing',
          page: page.name, pageId: page.id,
          title: expected.name + ' has a page but no component on it',
          detail: 'The page documents "' + expected.name + '", but nothing on it is a COMPONENT or ' +
            'COMPONENT_SET. Either the component was deleted, or it was never built here.'
        });
      } else if (!docFrame) {
        findings.push({
          severity: 'structure',
          page: page.name, pageId: page.id,
          title: expected.name + ' has no doc frame named "' + wantStyled + '"',
          detail: 'Every component page is one doc frame named after the component. Found: ' +
            (children.length ? children.slice(0, 4).map(c => '"' + c.name + '"').join(', ') : 'an empty page') + '.'
        });
      }

      // A footer is required on component pages and forbidden on token pages.
      if (docFrame && 'children' in docFrame) {
        const kids = docFrame.children || [];
        const footerAt = kids.map(k => k.name).lastIndexOf('.Component Page Footer');
        if (footerAt === -1) {
          findings.push({
            severity: 'structure', page: page.name, pageId: page.id,
            title: expected.name + ' is missing its .Component Page Footer',
            detail: 'The footer must be the last child of the doc frame. A missing footer usually ' +
              'traces back to the (Template) page it was cloned from.'
          });
        } else if (footerAt !== kids.length - 1) {
          findings.push({
            severity: 'structure', page: page.name, pageId: page.id,
            title: expected.name + '’s footer is not the last child',
            detail: 'Found at position ' + (footerAt + 1) + ' of ' + kids.length + '.'
          });
        }
      }
    }

    // Token pages must NOT carry a footer.
    for (const page of pages) {
      if (!kitIsTokenPage(page.name)) continue;
      if (isStale(gen)) return;
      await ensurePageLoaded(page);
      let offender = null;
      const hunt = (node, depth) => {
        if (offender || depth > 3) return;
        if (node.name === '.Component Page Footer') { offender = node; return; }
        if ('children' in node) for (const child of node.children) hunt(child, depth + 1);
      };
      for (const child of (page.children || [])) hunt(child, 0);
      if (offender) {
        findings.push({
          severity: 'structure', page: page.name, pageId: page.id,
          title: page.name + ' carries a .Component Page Footer',
          detail: 'Colors, Typography, Icons and Shadows document tokens, not components, and must ' +
            'not have one. A fixed-height footer here silently pads the page.'
        });
      }
    }

    // Community coverage: every Nova component except the five chat primitives.
    const proOnlyPresent = [];
    if (kit.edition === 'Community') {
      for (const record of components) {
        if (kitIsProOnly(record.name)) proOnlyPresent.push(record.name);
      }
    }

    findings.sort((a, b) => {
      const rank = { missing: 0, structure: 1 };
    ...
    figma.ui.postMessage({
      ...
      report: {
        ...
        missingCount: findings.filter(f => f.severity === 'missing').length,
        proOnlyPresent,
        scannedAt: Date.now()
      }
    });
  } catch (error) {
    console.error('Kit audit failed', error);
    figma.ui.postMessage({ type: 'kit-report', report: null, error: describeError(error) });
  } finally {
    endProgress();
  }
}
...
async function applyRef(ref, options) {
  ...
  // Some references cannot be matched because the thing they point at is simply
  // not in this file — no matcher can help, and saying "no local match" sends the
  // user hunting for something that does not exist.
  const structural = kitUnfixableReason(ref.sourceName);
  if (structural) {
    return { ok: false, kind: kindBucket, kitBlocked: structural.code, message: structural.message };
  }
...
function planForRefs(refs, threshold) {
  ...
  for (const ref of refs) {
    ...
    const structuralBlock = kitUnfixableReason(ref.sourceName);
    if (structuralBlock) {
      plan.blocked.push(planRow(ref, null, structuralBlock.message));
      continue;
    }
    const match = ref.kind === 'component'
      ? matchComponentByNames(
...
// ── JSON report export ───────────────────────────────────────────────────────
// A portable snapshot of every scanned page's groups, meant to be handed to
// someone (or something) working outside this plugin — the AI-facing "todo
// list": each group already carries `blockedReason` in plain English when it
// can't be auto-applied, and `match` when it can. Pulls from `pageReports`
// (the in-memory full reports for pages scanned THIS session) rather than the
// persisted index, which only holds counts. Scan first (Scan file / Scan all)
// so there is something to export.
...
function buildJsonReport() {
  ...
  for (const [pageId, report] of pageReports.entries()) {
    const page = findPage(pageId);
    const pageName = page ? page.name : pageId;
    remoteTotal += report.remoteTotal;
    byKind.component += report.byKind.component;
    byKind.style += report.byKind.style;
    byKind.variable += report.byKind.variable;
    ...
    pages.push({
      ...
      groups: report.groups.map(function (g) {
        return {
          sourceName: g.sourceName,
          kind: g.kind,
          total: g.total,
          match: g.match ? {
            targetName: g.match.targetName,
            confidence: g.match.confidence,
            how: g.match.how,
            ambiguous: !!g.match.ambiguous
          } : null,
          broken: g.broken || null,
          manualOnly: !!g.manualOnly,
          unfixable: !!g.unfixable,
          partiallyBroken: !!g.partiallyBroken,
  ...
  pages.sort(function (a, b) { return b.remoteTotal - a.remoteTotal; });

  return {
    kind: 'obra-library-scanner-shadcn-report',
    version: 1,
...
function handleExportJsonReport() {
  const report = buildJsonReport();
  if (report.pagesScanned === 0) {
    figma.notify('Nothing scanned yet — run Scan file (or Scan all) first.', { error: true });
    return;
  }
  const safeName = String(figma.root.name || 'report').replace(/[^\w.-]+/g, '-');
  figma.ui.postMessage({
    type: 'json-report-ready',
    json: JSON.stringify(report, null, 2),
    fileName: safeName + '-remote-refs.json'
  });
  figma.notify('Exported ' + report.remoteTotal + ' reference(s) across ' + report.pagesScanned + ' page(s).');
}
...
figma.ui.onmessage = async (msg) => {
  try {
    switch (msg.type) {
      ...
      case 'export-json-report':
        handleExportJsonReport();
        break;
      ...
      case 'fix-in-main': {
        ...
        // Scan the COMPONENT, not its page. Resolving the node loads its page
        // implicitly, and a component subtree is a few hundred nodes where a Pro
        // Blocks page is tens of thousands — scanning the page to reach the
        // component is why this failed on large pages.
        const scopeKey = 'component:' + ownerId;
        const report = await scanRoots({
          roots: [owner],
          gen,
          scopeKey,
          ...
          onPhase: (message, processed, total) => postProgress({
            message, processed: processed || 0, total: total || 0,
            pageName: owner.name, cancellable: true
          })
        });
        if (!report) { endProgress(); break; }

        if (!report.remoteTotal) {
          endProgress();
          figma.ui.postMessage({
            type: 'toast', kind: 'info',
            message: 'No remote references inside ' + owner.name + '.'
          });
          break;
        }

        await handleApplyFixes([scopeKey], {});
        // The component changed, so every page holding an instance of it now
        // shows something its cached report does not describe.
        pageReports.delete(mainPage.id);
        markPageDirty(mainPage.id);
        break;
      }
